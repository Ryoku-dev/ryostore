...
State lives in `$XDG_STATE_HOME/ryoku/plugins/awe-habits/state.json` (one JSON document, created with `mkdir -p` on first save). Tile position and scale are not stored here; the shell owns placement and sizing.
